...
for i in range(len(froml)): froml[i] = froml[i].upper()
for i in range(len(tol)): tol[i] = tol[i].upper()
...
for i in range(len(froml)):
    for j in range(len(tol)):
        if tol[j].upper() == froml[i].upper():
            if maplist[i] != -1:
                print("Error - Duplicate: Being overwritten at", i, "to", j)
...
print("unmatched in tol:")
for i in range(len(maplist)):
    if not i in maplist:
        print (i, tol[i])
#print(tol[13], tol[24])

print("unmatched in froml:")
for i in range(len(maplist)):
    if maplist[i] == -1:
        print(i, froml[i])
